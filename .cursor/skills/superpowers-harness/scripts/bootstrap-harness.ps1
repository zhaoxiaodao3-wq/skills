...
$cliContent = Get-Content $CliSrc -Raw -Encoding UTF8
...
if (-not (Test-Path $AgentsDst)) {
    $Template = Get-Content (Join-Path $HarnessRoot "references\AGENTS.md.template") -Raw -Encoding UTF8
...
$Snippet = Get-Content (Join-Path $HarnessRoot "references\cursorrules.snippet") -Raw -Encoding UTF8
if (Test-Path $CursorRules) {
    $Existing = Get-Content $CursorRules -Raw -Encoding UTF8
    if ($Existing -notmatch 'Harness 门禁') {
        Add-Content -Path $CursorRules -Value "`n$Snippet" -Encoding UTF8
...
if (Test-Path $Gitignore) {
    ...
    if ($Gi -notmatch '\.harness/') {
        Add-Content -Path $Gitignore -Value "`n.harness/" -Encoding UTF8
