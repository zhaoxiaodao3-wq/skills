...
const CONFIG_FILE = process.env.SKILL_ROUTING_CONFIG_FILE
  ? path.resolve(process.env.SKILL_ROUTING_CONFIG_FILE)
...
export function readConfig() {
  const empty = { routingMdPath: '', skillsRoot: '', defaultSkillsRoot: '', updatedAt: '' };
  if (!fs.existsSync(CONFIG_FILE)) return { ...empty };
  try {
    const raw = JSON.parse(fs.readFileSync(CONFIG_FILE, 'utf8'));
    const skillsRoot = typeof raw.skillsRoot === 'string' ? raw.skillsRoot : '';
    const defaultSkillsRoot =
      typeof raw.defaultSkillsRoot === 'string' && raw.defaultSkillsRoot.trim()
        ? raw.defaultSkillsRoot
        : skillsRoot;
    return {
...
export function writeConfig(partial = {}) {
  ...
  const skillsRoot =
    partial.skillsRoot !== undefined ? String(partial.skillsRoot || '') : cur.skillsRoot;
  let defaultSkillsRoot =
    partial.defaultSkillsRoot !== undefined
      ? String(partial.defaultSkillsRoot || '')
      : cur.defaultSkillsRoot;
  if (!defaultSkillsRoot && skillsRoot) defaultSkillsRoot = skillsRoot;
  const next = {
    routingMdPath:
      partial.routingMdPath !== undefined ? String(partial.routingMdPath || '') : cur.routingMdPath,
    skillsRoot,
    defaultSkillsRoot,
